...
namespace Engine {
	...
	void ProjectAssetDocumentPatch::PatchJsonAssetName(const std::filesystem::path& path, AssetType type, bool) {
	...
	void ProjectAssetDocumentPatch::PatchDuplicatedJsonAsset(const std::filesystem::path& path, AssetType type) {
		// 複製されたアセットの表示名を更新する、新しいGUIDは.meta作成時に発行される
		PatchJsonAssetName(path, type, true);
	}

	void ProjectAssetDocumentPatch::PatchRenamedJsonAsset(const std::filesystem::path& path, AssetType type) {
		// リネームされたアセットの名前のみを更新しGUIDは維持
		PatchJsonAssetName(path, type, false);
	}

	void ProjectAssetDocumentPatch::PatchDuplicatedDirectoryAssets(const std::filesystem::path& duplicatedDirectory) {
		std::error_code ec;
