...
namespace Engine {
	...
	const char* ProjectAssetDocumentFactory::GetFileSuffix(ProjectAssetFileKind kind) {
		// ファイルの種類に応じた拡張子を返す
		switch (kind) {
	...
	std::string ProjectAssetDocumentFactory::BuildFileContent(ProjectAssetFileKind kind, const std::string& assetName) {
		...
		case ProjectAssetFileKind::Script:
		{
			...
			const std::string className = ProjectAssetPath::MakeCSharpClassName(assetName);
			return std::format(
				"using NEMEngine;\n"
				"\n"
				"namespace {0};\n"
	...
	bool ProjectAssetDocumentFactory::WriteTextFile(const std::filesystem::path& path, const std::string& content) {
	...
	std::string ProjectAssetDocumentFactory::LoadGameScriptRootNamespace() {
