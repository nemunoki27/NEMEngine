...
public sealed partial class LineRenderer {
    ...
    public DynamicBuffer<LineRendererPoint> Points =>
        gameObject.GetBuffer<LineRendererPoint>();

    public MaterialInstance MaterialInstance =>
        new(gameObject, RendererMaterialTarget.Line);
    ...
    public LinePoint AddPoint(LinePoint point) {
        point.index = NativeRenderingAPI.LineAddComponentPoint(native, point);
        return point;
    }
    ...
    public void UpdatePoint(LinePoint point) {
        NativeRenderingAPI.LineUpdateComponentPoint(native, point);
    }

    // 点列を差し替える。loopで始点と終点をつなぐ
    public void SetPoints(ReadOnlySpan<LinePoint> points, bool loop = false) {
        NativeRenderingAPI.LineSetComponentPoints(native, points, loop);
    }

    // 点列をクリアする
    public void Clear() {
        NativeRenderingAPI.LineSetComponentPoints(native, ReadOnlySpan<LinePoint>.Empty, false);
    }
}
