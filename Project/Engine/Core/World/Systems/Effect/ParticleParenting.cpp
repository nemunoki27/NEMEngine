...
namespace {
	...
	void AttachParticleParent(Engine::Particle& particle, const Engine::Matrix4x4& parentMatrix,
		...
		particle.spawnDirection = Engine::Vector3::NormalizeOr(
			Engine::Vector3::TransferNormal(particle.spawnDirection, inverseParent), Engine::Vector3(0.0f, 1.0f, 0.0f));
...
//============================================================================
const Engine::ParticlePhaseParentSettings& Engine::ParticleParenting::ResolveParticleParentSettings(
	const ParticlePhaseDefinition& phase, const ParticlePhaseParentSettings& parentSettings,
	bool useAssetParentSettings) {

	return useAssetParentSettings ? phase.parentSettings : parentSettings;
}

void Engine::ParticleParenting::UpdateParticleParent(Particle& particle,
	const ParticlePhaseParentSettings& settings, const ParticleParentPose& parent, bool preserveWorldRotationScale) {
...
void Engine::ParticleParenting::UpdateParticleParents(std::vector<Particle>& particles,
	...
	bool useAssetParentSettings, const std::vector<ParticleParentPose>& parents) {

	for (Particle& particle : particles) {

		if (particle.phaseIndex < group.phases.size()) {
			const ParticlePhaseDefinition& phase = group.phases[particle.phaseIndex];
...
void Engine::ParticleParenting::RefreshParticleWorldTransform( Particle& particle, const ParticleParentPose* parent) {
