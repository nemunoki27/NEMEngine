...
namespace {
	...
	const Engine::AnimationPropertyValue* FindEvaluatedValue(
		std::span<const Engine::AnimationEvaluatedValue> values,
		const Engine::AnimationPropertyBinding& binding) {
...
void Engine::AnimationClipEvaluator::ApplyClip(ECSWorld& world, const Entity& entity,
	...
	if (clip.relativeTransform) {
		...
		ComposeRelativeTransform(values, baseValues);
