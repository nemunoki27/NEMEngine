...
// 平行光源計算
float3 EvaluateDirectionalLight(DirectionalLight light, float3 worldPos, float3 worldNormal) {
	...
	float shadow = TraceDirectionalShadow(worldPos, worldNormal, light.direction) ? 0.0f : 1.0f;
	return light.color.rgb * (light.intensity * ndotl * shadow);
}
// 点光源計算
...
PSOutput main(VSOutput input) {
	...
	for (uint i = 0; i < directionalCount; ++i) {
		...
		lighting += EvaluateDirectionalLight(gDirectionalLights[i], input.worldPos, worldNormal);
	}
